...
function processSum(numberList, callback) {
  /* CODE HERE */
  const sumtotal= numberList.reduce(function(accumulator,currentvalue){
    return accumulator + currentvalue
  },0);

  return callback(sumtotal);
}
...
function processContains(item, list, callback) {
  /* CODE HERE */
  const truth = list.includes(item);
...
function processDuplicateFree(list, callback) {
  ...
  //       if(dedupped.indexOf(list[i]) == -1){
  //           dedupped.push(list[i])
  //       }
  //   }

  let dedupped = Array.from(new Set(list))
  return callback(dedupped);
...
function getFullNames(runners) {
  ...
  runners.forEach(function(items){
    return shirtOrder.push(`${items.last_name}, ${items.first_name}`);
  });
...
function firstNamesAllCaps(runners) {
  ...
  const cappedName = runners.map(function(element){
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  const tShirtFilter = runners.filter(function(item){
